...
def register():
    ...
    return jsonify(
        {"message": "Registration successful! Please login.", "status": "success"}
    )
...
def get_profile():
    ...
    return jsonify({"profile": dict(profile), "status": "success"})
...
def get_haul_status():
    ...
    try:
        ...
        if not haul_start_str:
            return jsonify({"has_haul": False, "message": "Haul start date not set."})
            ...
            return jsonify(
                {
                    ...
                    "progress_percent": round(
                        (days_since_haul / lunar_year_days) * 100, 1
                    ),
                    "status": "success",
                }
            )
    ...
    return jsonify(
        {
            ...
            "average_per_donor": round(
                total_predicted / len(donors) if donors else 0, 2
            ),
...
def get_segments():
        ...
        segment_list.append(
            {
                ...
                "avg_zakat": (
                    round(data["total_zakat"] / data["count"], 2)
                    if data["count"] > 0
                    else 0
                ),
                "avg_wealth": (
                    round(data["total_wealth"] / data["count"], 2)
                    if data["count"] > 0
                    else 0
                ),
            }
    ...
    return jsonify(
        {"tier_counts": tier_counts, "segments": segment_list, "status": "success"}
    )
...
def get_trends():
    ...
    cursor.execute(
        "SELECT income, total_wealth, zakat_amount, donor_tier, employment_status FROM donors"
    )
    donors = cursor.fetchall()
    conn.close()
...
def get_at_risk():
    ...
    for d in at_risk:
        days_since = (
            datetime.now() - datetime.strptime(d["last_payment_date"], "%Y-%m-%d")
        ).days
        at_risk_list.append(
            {
                "DonorID": d["donor_id"],
...
def get_data():
    ...
    scatter_data = [
        {"Income": d["income"], "ZakatAmount": d["zakat_amount"]} for d in sample
    ]
