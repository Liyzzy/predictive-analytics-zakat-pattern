    ...
    for _, row in df.iterrows():
        try:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                row['DonorID'], row['Age'], row['Income'], row['Savings'],
                row['GoldValue'], row['InvestmentValue'], row['TotalWealth'],
                row['FamilySize'], row['EmploymentStatus'], row['PreviousContributionScore'],
                row['LastPaymentDate'], row['HaulStartDate'], row['DonorTier'], row['ZakatAmount']
            ))
