...
async function fetchDashboardData() {
    try {
        ...
        document.getElementById('totalZakat').innerText = formatCurrency(data.total_zakat_pool);
        document.getElementById('avgZakat').innerText = formatCurrency(data.total_zakat_pool / data.total_donors);
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Avg Zakat (RM)',
                data: values,
                backgroundColor: ['rgba(244, 63, 94, 0.7)', 'rgba(6, 182, 212, 0.7)', 'rgba(139, 92, 246, 0.7)'],
                borderColor: ['rgba(244, 63, 94, 1)', 'rgba(6, 182, 212, 1)', 'rgba(139, 92, 246, 1)'],
                borderWidth: 1
            }]
        },
        options: {
...
function setupPredictionForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const payload = {
            ...
            PreviousContributionScore: Number(document.getElementById('p_history').value)
        };
        ...
        try {
            ...
            if (result.status === 'success') {
                document.getElementById('resultAmount').innerText = formatCurrency(result.predicted_zakat);
...
function formatCurrency(num) {
    return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'MYR' }).format(num);
}
